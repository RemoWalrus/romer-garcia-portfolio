  ...
  const generateCharacter = async () => {
    ...
    try {
      ...
      const androidFaceDescription = gender === "other"
        ? "This synthetic android has an androgynous humanoid form with a distinctive PURE WHITE, PALE, PORCELAIN-LIKE face plate made of 1-4 smooth panels (NO warm tones, NO golden/bronze hues - only cool white/pale colors). The face has gender-neutral features with soft, balanced contours, subtle nose and mouth. The android has neon blue illuminated cybernetic eyes. Body/armor is COOL METALLIC - silver, chrome, steel tones (NO golden or bronze colors). It may have animal-like add-ons (tail, ears, enhanced limbs) or vehicle-like components (wheels, thrusters, mechanical appendages) integrated into its design. " + (Math.random() > 0.5 ? "Synthetic hair or fiber-optic cables cascade from the head area." : "Sleek cables or synthetic hair strands are integrated into the head design.")
